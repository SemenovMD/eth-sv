# Ethernet
Данный модуль представляет собой реализацию `Ethernet` на `SystemVerilog`. Модуль поддерживает обработку `UDP/IP`, `ARP` и `ICMP`. Разработан для интеграции в FPGA, включает FIFO, проверку CRC на каждом уровне сетевой модели, а также интегрирован в `AXI-Stream`.
